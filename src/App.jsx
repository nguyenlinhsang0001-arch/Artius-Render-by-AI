...
const APP_VERSION = "v34.5 · 19/06/2026";
...
<body>
  ...
  const topBarEl = (
    ...
    >
      <div className="w-full max-w-[1536px] mx-auto px-3 md:px-5 h-14 md:h-16 flex items-center gap-3">
        <div className="flex items-center gap-2.5 min-w-0">
          <img src={ARTIUS_LOGO} alt="ARTIUS" className="h-7 md:h-8 w-auto shrink-0" style={{ opacity: 0.95 }} />
          <div className="hidden sm:flex items-center gap-2 min-w-0">
            <span className="text-sm md:text-[15px] font-semibold tracking-tight whitespace-nowrap" style={{ color: C.text }}>
              Interior Render Agent
            </span>
            <span
              className="rounded-full px-2 py-0.5 text-[10px] font-bold tracking-wide whitespace-nowrap"
              style={{ background: `${C.accent}1f`, border: `1px solid ${C.accent}55`, color: C.accentSoft }}
              title="Chỉ dùng để tìm ý tưởng — không nên hiệu chỉnh chi tiết"
            >
              Ý TƯỞNG
            </span>
          </div>
        ...
        <div className="flex items-center gap-2 md:gap-3 shrink-0">
          ...
          <div className="hidden md:flex items-center gap-3">
            {/* pill counts */}
            <div className="flex items-center gap-3 rounded-xl" style={{ background: C.inputBg, border: `1px solid ${C.line}`, padding: "7px 15px" }}>
              <div className="flex items-center gap-1.5">
                <span style={{ fontSize: 9.5, letterSpacing: "0.14em", color: C.textDim, fontWeight: 700 }}>PROMPT</span>
                <span style={{ fontSize: 15, fontWeight: 600, color: C.text, fontFamily: MONO, lineHeight: 1 }}>{counts.prompts.toLocaleString()}</span>
              </div>
              <div style={{ width: 1, height: 18, background: C.line }} />
              <div className="flex items-center gap-1.5">
                <span style={{ fontSize: 9.5, letterSpacing: "0.14em", color: C.textDim, fontWeight: 700 }}>ẢNH</span>
                <span style={{ fontSize: 15, fontWeight: 600, color: C.text, fontFamily: MONO, lineHeight: 1 }}>{counts.images.toLocaleString()}</span>
              </div>
            </div>
  ...
  const navRailEl = (
    <nav
      className="hidden md:flex flex-col items-center gap-2 py-4 shrink-0"
      ...
      {navRailItems.map(({ Icon, label, key }) => {
        ...
        return (
          <button
            ...
            style={{ color: active ? C.accent : C.railIcon, cursor: "pointer", opacity: active ? 1 : 0.85 }}
          >
            ...
            <span className="flex items-center justify-center rounded-xl" style={{ width: 38, height: 38, background: active ? `${C.accent}1a` : "transparent", border: active ? `1px solid ${C.accent}40` : "1px solid transparent" }}>
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: `radial-gradient(140% 90% at 50% -10%, ${C.bgGrad} 0%, ${C.bgDeep} 60%)`, color: C.text, fontFamily: FONT, overflowX: "clip" }}>
      ...
      <style>{`
        ...
        *:focus-visible { outline: 2px solid ${C.accent}; outline-offset: 2px; }
        ...
        @media (min-width: 768px) {
          .ipa-grid { display: grid; grid-template-columns: 396px minmax(0, 1fr); gap: 1.25rem; align-items: stretch; }
      ...
      <div className="w-full max-w-[1536px] md:max-w-none mx-auto px-4 md:px-6 py-4 md:py-6">
        ...
        <div className="py-2 mb-2">
          ...
          {/* A2: nút Phân tích & Tạo prompt — cố định đáy panel trái (desktop) */}
          <div className="hidden md:block sticky bottom-0 z-10 mt-4 pt-3 pb-1" style={{ background: `linear-gradient(to top, ${C.bgDeep} 72%, transparent)` }}>
            {actionButton}
          </div>
          </div>{/* /CỘT TRÁI */}
          ...
          <div className={(activeTab === "result" ? "" : "hidden ") + "ipa-col-right"}>
            ...
            <div className="hidden md:flex items-center justify-between mb-3 pb-3" style={{ borderBottom: `1px solid ${C.line}` }}>
              <div className="flex items-center gap-2 text-sm">
                <span className="font-semibold" style={{ color: C.text }}>Khung làm việc</span>
                <span style={{ color: C.textFaint }}>·</span>
                <span style={{ color: C.textDim }}>{(prompts || genImg) ? "có kết quả" : "trống"}</span>
              </div>
              <div className="flex items-center gap-1.5 text-[11px]" style={{ color: C.textDim, fontFamily: MONO }}>
                <span className="rounded-md px-2 py-1" style={{ background: C.inputBg, border: `1px solid ${C.line}` }}>{aspectRatio}</span>
                <span className="rounded-md px-2 py-1" style={{ background: C.inputBg, border: `1px solid ${C.line}` }}>Chuẩn</span>
              </div>
            </div>
            {/* C: empty hero (desktop) — khi chưa có prompt/ảnh */}
            {!prompts && !genImg && !progressActive && (
              <div className="hidden md:flex flex-col items-center justify-center text-center py-24 px-6 ipa-anim">
            ...
            {prompts && (
              <div className="ipa-anim">
                ...
                {/* B2: composer + nút Tạo render (desktop) */}
                <div className="hidden md:flex items-stretch justify-end gap-2 mt-5 pt-4" style={{ borderTop: `1px solid ${C.line}` }}>
                  {prompts?.nanobanana ? (
                  <div className="flex-1 rounded-xl px-3 py-2.5 text-[12.5px] leading-snug max-h-24 overflow-y-auto ipa-scroll" style={{ background: C.inputBg, border: `1px solid ${C.line}`, color: C.text, fontFamily: MONO, whiteSpace: "pre-wrap" }}>
                    {prompts.nanobanana}
                  </div>
                  ) : null}
                  <button
                    type="button"
                    onClick={renderImage}
                    disabled={!(prompts?.nanobanana && modelImg) || genBusy}
                    className="shrink-0 inline-flex items-center gap-2 rounded-xl px-4 text-sm font-semibold transition-all"
                    style={{
                      background: (prompts?.nanobanana && modelImg && !genBusy) ? C.accent : C.panel2,
                      color: (prompts?.nanobanana && modelImg && !genBusy) ? C.onAccent : C.textDim,
                      border: `1px solid ${(prompts?.nanobanana && modelImg && !genBusy) ? C.accent : C.line}`,
                      cursor: (prompts?.nanobanana && modelImg && !genBusy) ? "pointer" : "not-allowed",
                    }}
                  >
                    {genBusy ? (<><Loader2 className="w-4 h-4 animate-spin" /> Đang tạo…</>) : (<><ImageIcon className="w-4 h-4" /> Tạo render</>)}
                  </button>
                </div>
