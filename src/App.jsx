    ...
    <div className="min-h-screen p-4 md:p-8 overflow-x-hidden" style={{ background: `radial-gradient(120% 75% at 50% -8%, ${C.bgGrad} 0%, ${C.bg} 55%)`, color: C.text, fontFamily: FONT }}>
      ...
      <div className="w-full max-w-[1536px] mx-auto">
        ...
        <div className="sticky top-0 z-30 py-2 mb-2" style={{ background: "rgba(14,17,24,0)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)" }}>
          <div className={`grid ${isDesktop ? "grid-cols-2" : "grid-cols-3"} gap-1.5 rounded-xl p-1 h-12`} style={{ background: C.panel2, border: `1px solid ${C.line}` }}>
            {[{ id: "src", label: "Nguồn & phong cách" }, { id: "cfg", label: "Thiết lập & điều chỉnh" }, { id: "result", label: "Kết quả" }]
              ...
              return (
                <button
                  ...
                  className="relative flex items-center justify-center rounded-lg px-2 py-2 text-xs sm:text-sm font-semibold leading-tight transition-all duration-200"
                  style={on
          ...
          <div className={(activeTab === "result" ? "" : "hidden ") + "ipa-col-right"}>
            ...
            {/* PROMPT RENDER — đưa lên trên cùng khu Kết quả */}
            {prompts && (
            ...
            {/* Đường kẻ ngăn cách giữa Prompt và Image */}
            {platform === "nanobanana" && prompts?.nanobanana && modelImg && (
              <div className="my-6" style={{ borderTop: `1px solid ${C.line}` }} />
            )}

            {/* IMAGE — xuống dưới Prompt, dính theo cuộn (sticky) */}
                {/* === RENDER ẢNH bằng gpt-image-2 (ChatGPT) — chỉ Nano Banana + có MODEL === */}
                {platform === "nanobanana" && prompts?.nanobanana && modelImg && (
                  <div className="mt-3 rounded-2xl p-4" style={{ background: C.panel, border: `1px solid ${C.line}`, position: "sticky", top: "0.5rem", zIndex: 5 }}>
                    <div className="flex items-center justify-between mb-2.5 gap-2 flex-wrap">
                      <div>
