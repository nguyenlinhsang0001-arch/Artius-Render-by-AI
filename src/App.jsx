...
import { Loader2, Copy, Check, Image as ImageIcon, Sparkles, AlertCircle, Palette, Box, Lock, Pencil, Move3d, Droplet, Layers, Maximize2, Ban, X, RefreshCw, Home, History, ChevronDown, Shuffle, RotateCcw, Download } from "lucide-react";
...
const APP_VERSION = "v25.0 · 09/06/2026";
...
export default function InteriorPromptAgent() {
  ...
  const [activeTab, setActiveTab] = useState("setup"); // P1: tab toàn trang "setup" (Thiết lập) | "result" (Kết quả)
    ...
    <div className="min-h-screen p-4 md:p-8 overflow-x-hidden" style={{ background: `radial-gradient(120% 75% at 50% -8%, ${C.bgGrad} 0%, ${C.bg} 55%)`, color: C.text, fontFamily: FONT }}>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="sticky top-0 z-30 py-2 mb-2" style={{ background: "rgba(11,14,19,0.85)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)" }}>
          <div className="grid grid-cols-2 gap-1.5 rounded-xl p-1" style={{ background: C.panel2, border: `1px solid ${C.line}` }}>
            {[{ id: "setup", label: "① Thiết lập" }, { id: "result", label: "② Kết quả" }].map((t) => {
        ...
        <div className="grid grid-cols-1 gap-6 mt-4 items-start">
          ...
          <div>
            ...
            <div className="mt-4">
              ...
              {!prompts && (
                <button
                  onClick={() => { analyze(); setActiveTab("result"); }}
                  disabled={!canAnalyze}
              ...
              {/* THANH TIẾN TRÌNH đã chuyển lên global (dưới thanh tab) — luôn thấy ở mọi tab. */}
            </div>
