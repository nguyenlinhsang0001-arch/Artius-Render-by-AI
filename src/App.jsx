...
Return ONLY a valid JSON object (no markdown/backticks): {"prompt": "the English render prompt"}. Nothing outside the JSON.`;
  ...
  async function renderImage() {
    ...
    // Ảnh input: MODEL trước (nền/geometry), STYLE sau (tham chiếu phong cách).
    const images = [{ data: modelImg.data, mediaType: modelImg.mediaType }];
...
<body>
  ...
  const actionBtnArmed = armAnalyze && actionBtnReAnalyze; // chỉ "Phân tích" (tốn token) cần xác nhận 2 chạm
  ...
  const runActionBtn = () => {
    ...
    if (actionBtnReAnalyze) {
      if (armAnalyze) { setArmAnalyze(false); analyze(prompts ? pendingChanges : undefined); goResult(); }
      else { setArmAnalyze(true); setTimeout(() => setArmAnalyze(false), 3000); } // chờ xác nhận, tự hủy 3s
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 overflow-x-hidden" style={{ background: `radial-gradient(120% 75% at 50% -8%, ${C.bgGrad} 0%, ${C.bg} 55%)`, color: C.text, fontFamily: FONT }}>
      ...
      <style>{`
        ...
        *:focus-visible { outline: 2px solid ${C.accent}; outline-offset: 2px; }
        ...
        @media (min-width: 768px) {
          .ipa-grid { display: grid; grid-template-columns: 7fr 5fr; gap: 1rem; align-items: start; }
      ...
      <div className="w-full">
        ...
        <div className="sticky top-0 z-32 py-2 mb-2" style={{ background: "rgba(14,17,24,0)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)" }}>
            ...
            {[{ id: "src", label: "Nguồn & phong cách" }, { id: "cfg", label: "Thiết lập & điều chỉnh" }, { id: "result", label: "Kết quả" }]
              ...
              return (
                <button
                  ...
                  className="relative rounded-lg px-2 py-2 text-xs sm:text-sm font-semibold leading-tight transition-all duration-200"
                  style={on
          ...
          <div>
            ...
            {!analysis && (
              <div className="rounded-2xl px-4 py-3 mt-1 flex items-center justify-between gap-2 cursor-not-allowed" style={{ background: C.panel, border: `1px solid ${C.line}`, opacity: 0.45 }} aria-disabled="true">
                <span className="text-lg flex items-center gap-2 font-bold tracking-tight" style={{ color: C.text }}>
                  <Palette className="w-4 h-4" style={{ color: C.accent }} /> Điều Chỉnh Nâng Cao
                </span>
          ...
          <div className={(activeTab === "result" ? "" : "hidden ") + "ipa-col-right"}>
            ...
            {/* TẠO ẢNH THỬ — đưa lên trên cùng khu Kết quả */}
                    ...
                    <div className="flex items-center justify-between mb-2.5 gap-2 flex-wrap">
                      ...
                      >
                        {genStatus === "generating"
                          ...
                          : genImg
                            ? (<><ImageIcon className="w-4 h-4" /> Tạo lại</>)
                    ...
                    {genImg && genStatus !== "generating" && (
                      <div>
                        ...
                        <div
                          ...
                          style={{ border: `1px solid ${C.line}`, touchAction: "none", cursor: "pointer" }}
                        ...
                        >
                          ...
                          <img src={genImg} alt="Kết quả gpt-image-2" className="block w-full" draggable={false} />
            ...
            {/* Đường kẻ ngăn cách giữa Tạo ảnh và Prompt */}
            {platform === "nanobanana" && prompts?.nanobanana && modelImg && (
              <div className="my-6" style={{ borderTop: `1px solid ${C.line}` }} />
            )}

            {/* PROMPT RENDER — đưa xuống dưới Tạo ảnh */}
            {prompts && (
              <div className="ipa-anim">
                              ...
                              <div className="mt-2 pt-2 space-y-2.5" style={{ borderTop: `1px solid ${C.lineSoft}` }}>
                                {h.genImg && (
                                  <div>
                                    ...
                                    <img src={h.genImg} alt="Ảnh đã lưu" className="mt-1 w-full rounded" style={{ border: `1px solid ${C.lineSoft}` }} />
                                  </div>
                                )}
