...
import { Loader2, Copy, Check, Image as ImageIcon, Sparkles, AlertCircle, Palette, Box, Lock, Pencil, Move3d, Droplet, Layers, Maximize2, Ban, X, RefreshCw, Home, History, ChevronDown, ChevronLeft, ChevronRight, Shuffle, RotateCcw, Download, ZoomIn, Building2 } from "lucide-react";
...
const APP_VERSION = "v32.7 · 14/06/2026";
...
export default function InteriorPromptAgent() {
  ...
  // Ảnh minh họa độ áp style đang phóng to (lightbox). null = đóng.
  const [zoomIntensity, setZoomIntensity] = useState(null);
...
<body>
  ...
  return (
    <div className="min-h-screen p-4 md:p-8" style={{ background: `radial-gradient(120% 75% at 50% -8%, ${C.bgGrad} 0%, ${C.bg} 55%)`, color: C.text, fontFamily: FONT, overflowX: "clip" }}>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="rounded-2xl p-4" style={{ background: C.panel, border: `1px solid ${C.line}` }}>
                ...
                {/* Filter-strip: preview lớn (mức đang chọn) + strip 4 thumbnail là selector.
                    Ảnh render THẬT từ pipeline (public/intensity/lv0..3.webp), không phải CSS
                    filter. Nhãn "Minh họa" vì hiệu ứng thực tế còn tùy style preset. */}
                <div className="relative mt-3 rounded-xl overflow-hidden" style={{ border: `1px solid ${C.line}`, aspectRatio: "4 / 3" }}>
                  ...
                  <span className="absolute top-2 left-2 text-[10px] px-2 py-0.5 rounded-full" style={{ background: "rgba(11,14,19,0.66)", color: C.textDim, border: `1px solid ${C.lineSoft}`, backdropFilter: "blur(4px)" }}>Minh họa</span>
                  <button
                    onClick={() => setZoomIntensity(`/intensity/lv${styleIntensity}.webp`)}
                    aria-label="Phóng to ảnh minh họa"
                    className="absolute top-2 right-2 w-7 h-7 rounded-lg flex items-center justify-center transition-all"
                    style={{ background: "rgba(11,14,19,0.66)", color: C.accentSoft, border: `1px solid ${C.lineSoft}`, backdropFilter: "blur(4px)" }}
                  >
                    <ZoomIn className="w-3.5 h-3.5" />
                  </button>
                  <span className="absolute bottom-2 left-2 text-[10px] px-2 py-0.5 rounded-full" style={{ background: "rgba(11,14,19,0.66)", color: C.accentSoft, border: `1px solid ${C.lineSoft}`, backdropFilter: "blur(4px)" }}>
                    {STYLE_INTENSITY_LEVELS[styleIntensity]?.label} · {STYLE_INTENSITY_LEVELS[styleIntensity]?.short}
                  </span>
                </div>

                {/* Strip 4 thumbnail = selector (thay 4 nút chữ cũ). */}
                <div className="grid grid-cols-4 gap-2 mt-3">
                  {STYLE_INTENSITY_LEVELS.map((lv) => {
                    const on = styleIntensity === lv.value;
                    return (
                      <button key={lv.value} onClick={() => changeStyleIntensity(lv.value)} className="text-center transition-all" aria-pressed={on}>
                        <div className="rounded-lg overflow-hidden" style={{ border: `2px solid ${on ? C.accent : "transparent"}`, aspectRatio: "1 / 1" }}>
                          <img src={`/intensity/lv${lv.value}.webp`} alt={lv.label} className="w-full h-full" style={{ objectFit: "cover" }} />
                        </div>
                        <div className="text-[10px] mt-1" style={{ color: on ? C.accentSoft : C.textDim, fontWeight: on ? 600 : 400 }}>{lv.short}</div>
                      </button>
                    );
                  })}
                </div>
                ...
                <p className="mt-1.5 text-[10px] leading-relaxed" style={{ color: C.textFaint }}>
                  Ảnh minh họa cho trục cường độ — hiệu ứng thực tế còn tùy style preset bạn chọn.
                </p>
              </div>
      ...
      {/* ===== LIGHTBOX phóng to ảnh minh họa Độ áp style ===== */}
      {zoomIntensity && (
        <div
          onClick={() => setZoomIntensity(null)}
          className="fixed inset-0 z-[100] flex items-center justify-center p-4 ipa-anim"
          style={{ background: "rgba(0,0,0,0.90)", backdropFilter: "blur(2px)" }}
        >
          <div className="relative w-full max-w-[920px]" onClick={(e) => e.stopPropagation()}>
            <img src={zoomIntensity} alt="Ảnh minh họa độ áp style" className="w-full rounded-xl" style={{ maxHeight: "90vh", objectFit: "contain" }} />
          </div>
        </div>
      )}
          </span>
