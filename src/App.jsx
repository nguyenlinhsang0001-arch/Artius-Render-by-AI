...
const APP_VERSION = "v36.9 · 21/06/2026";
...
export default function InteriorPromptAgent() {
  ...
  // Ctrl+Scroll zoom trên ô kết quả ảnh.
  useEffect(() => {
    ...
    const onWheel = (e) => {
      if (!e.ctrlKey) return;
      e.preventDefault();
      setImgZoom((prev) => Math.min(4, Math.max(0.5, prev - e.deltaY * 0.001)));
    };
  ...
  // Zoom ảnh kết quả: Ctrl+Scroll để phóng to/thu nhỏ (0.5x – 4x).
          ...
          <div className={(activeTab === "result" ? "" : "hidden ") + "ipa-col-right"}>
            ...
            {/* IMAGE — lên trên cùng khu Kết quả, dính theo cuộn (sticky) */}
                ...
                {platform === "nanobanana" && prompts?.nanobanana && modelImg && (!isDesktop || genImg || genStatus === "generating") && (
                  <div ref={resultPanelRef} className="mt-3 rounded-2xl p-3 ipa-img-sticky ipa-result-panel" style={{ background: C.bg, border: `1px solid ${C.line}` }}>
                    {!isDesktop && (
                    <div className="flex items-center justify-between mb-2.5 gap-2 flex-wrap">
                      <div>
                    ...
                    {genImg && genStatus !== "generating" && (
                      <div className="ipa-result-body">
                        {/* Toolbar: chọn chế độ so sánh + chỉ số zoom */}
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          <div className="flex items-center gap-1 rounded-lg p-1" style={{ background: C.panel2, border: `1px solid ${C.line}` }}>
                          ...
                          {/* Zoom indicator + nút reset — hiện khi zoom != 100% */}
                          <div className="flex items-center gap-1.5 ml-auto text-[11px]" style={{ color: C.textDim }}>
                            <ZoomIn className="w-3.5 h-3.5" />
                            <span style={{ fontVariantNumeric: "tabular-nums", minWidth: 32, textAlign: "right" }}>{Math.round(imgZoom * 100)}%</span>
                            {imgZoom !== 1 && (
                              <button
                                type="button"
                                ...
                                style={{ background: C.panel2, border: `1px solid ${C.line}`, color: C.accentSoft, cursor: "pointer" }}>
                                ↺
                              </button>
                            )}
                          </div>
                        ...
                        {/* Vùng ảnh — Ctrl+Scroll để zoom; overflow:hidden để clip */}
                        <div style={{ overflow: "hidden", borderRadius: 12 }}>
                          <div style={{ transform: `scale(${imgZoom})`, transformOrigin: "top center", transition: imgZoom === 1 ? "transform 0.2s" : "none" }}>
                        ...
                        {compareMode === "slider" ? (
                          <>
                            ...
                            <p className="text-[11px] mt-1.5 text-center" style={{ color: C.textFaint }}>Kéo thanh để so sánh · Ctrl+Scroll để zoom</p>
                          </>
                        ) : (() => {
