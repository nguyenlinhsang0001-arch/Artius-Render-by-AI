...
const APP_VERSION = "v36.3 · 21/06/2026";
      ...
      <style>{`
        ...
        *:focus-visible { outline: 2px solid ${C.accent}; outline-offset: 2px; }
        ...
        @media (min-width: 768px) {
          ...
          .ipa-grid {
            display: grid; grid-template-columns: 396px minmax(0, 1fr); grid-template-rows: minmax(0, 1fr); gap: 1.25rem;
          ...
          .ipa-pane-scroll { flex: 1 1 auto; min-height: 0; overflow-y: auto; padding-right: 4px; }
          ...
          .ipa-canvas-scroll { flex: 1 1 auto; min-height: 0; overflow-y: auto; padding-right: 4px; display: flex; flex-direction: column; }
        ...
        <div className="py-2 mb-2 md:py-0 md:mb-0">
          ...
          <div className={(activeTab === "result" ? "" : "hidden ") + "ipa-col-right"}>
            ...
            {!genImg && genStatus !== "generating" && !progressActive && (
              <div className="hidden md:flex flex-col items-center justify-center text-center py-4 px-4 ipa-anim">
                {modelImg ? (
                  <>
                    <img src={`data:${modelImg.mediaType};base64,${modelImg.data}`} alt="Ảnh MODEL" draggable={false} onContextMenu={(e) => e.preventDefault()} className="w-full max-h-[78vh] object-contain rounded-2xl select-none" style={{ border: `1px solid ${C.line}`, WebkitTouchCallout: "none", pointerEvents: "none" }} />
                    {!prompts && <p className="text-sm mt-4" style={{ color: C.textDim }}>Ảnh MODEL đã sẵn sàng — bấm <span style={{ color: C.accentSoft, fontWeight: 600 }}>“Phân tích &amp; Tạo prompt”</span> để bắt đầu.</p>}
                  </>
                ) : (
                  <>
            ...
            {/* IMAGE — lên trên cùng khu Kết quả, dính theo cuộn (sticky) */}
                ...
                {platform === "nanobanana" && prompts?.nanobanana && modelImg && (!isDesktop || genImg || genStatus === "generating" || genStatus === "error") && (
                  <div className="mt-3 rounded-2xl p-4 ipa-img-sticky md:ipa-result-panel" style={{ background: C.panel, border: `1px solid ${C.line}` }}>
                    {!isDesktop && (
                    <div className="flex items-center justify-between mb-2.5 gap-2 flex-wrap">
                      <div>
                    ...
                    {genImg && genStatus !== "generating" && (
                      <div className="md:ipa-result-body">
                        {/* Hold-to-compare: giữ (chuột/cảm ứng) trực tiếp trên ảnh -> lộ ảnh MODEL gốc; thả -> về ảnh AI */}
                        <div
                          className="relative rounded-xl overflow-hidden select-none md:ipa-result-frame"
                        ...
                        <div className="mt-2 flex items-center justify-between gap-2 flex-wrap md:ipa-result-caption">
                ...
                <div className="hidden md:block mt-4 pt-3" style={{ flex: "none" }}>
                  <div className="flex items-center gap-2 mb-2">
