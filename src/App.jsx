...
const APP_VERSION = "v32.4 · 14/06/2026";
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="rounded-2xl p-4" style={{ background: C.panel, border: `1px solid ${C.line}` }}>
                ...
                {/* Cường độ tổng: style intensity là MỘT scalar — 5 trục bề mặt
                    bên dưới được nâng ĐỀU theo mức (đúng STYLE_INTENSITY_CLAUSES
                    "tăng dần đều qua 4 bậc"), nên không fake mức lệch giữa các trục. */}
                <div className="flex items-center gap-2 mt-4">
                  <span className="text-[10px] uppercase tracking-wider shrink-0" style={{ width: 52, color: C.textFaint }}>Cường độ</span>
                  <div className="flex gap-1 flex-1">
                    {[0, 1, 2, 3].map((s) => (
                      <div key={s} className="h-1.5 flex-1 rounded-full" style={{ background: s <= styleIntensity ? C.accent : C.lineSoft }} />
                    ))}
                  </div>
                </div>

                {/* 5 trục bề mặt bị dial đồng thời — chỉ liệt kê, không gắn mức riêng. */}
                <div className="flex items-start gap-2 mt-2.5">
                  <span className="text-[10px] uppercase tracking-wider shrink-0 mt-0.5" style={{ width: 52, color: C.textFaint }}>Tác động</span>
                  <div className="flex flex-wrap gap-1.5 flex-1">
                    {["Màu", "Tương phản", "Ánh sáng", "Texture", "Mood"].map((d) => (
                      <span key={d} className="text-[10px] rounded px-1.5 py-0.5 leading-tight" style={{ background: C.panel2, border: `1px solid ${C.lineSoft}`, color: C.textDim }}>{d}</span>
                    ))}
                  </div>
                </div>
