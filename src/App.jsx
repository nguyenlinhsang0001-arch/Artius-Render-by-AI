...
- ${["Strictly preserve the MODEL's geometry, camera, walls, openings and every element's position; only swap surface materials, colors and lighting.",
      ...
      "Treat the MODEL as THIS room: keep its overall spatial layout and proportions as the basis, but you may choose a NEW camera angle and perspective and push the architectural detailing to the maximum. The result must still read as the same space, reinterpreted — not a different room."][geometry]}`
      : `\n\n${[
...
const APP_VERSION = "v28.17 · 11/06/2026";
...
export default function InteriorPromptAgent() {
  ...
  function effectivePlatformGuide() {
    ...
    if (geometry === 0) {
      ...
      guide = `a creative TEXT-TO-IMAGE generation prompt for gpt-image (NO base image, NOT using --params). This is the SAME room described in the analysis — preserve its room identity, spatial layout and proportions as the structural basis, but render it from a NEW camera angle and a fresh perspective (do NOT reuse the original viewpoint). You may push the architectural detailing to the maximum on top of that structure. ${styleClause} ${negNote} Produce a cohesive, photorealistic interior of THIS space seen from the new viewpoint.`;
    } else {
      const allowed = {
