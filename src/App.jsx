...
const GEOMETRY_LEVELS = [
  ...
  { value: 3, label: "Lấy cảm hứng",   short: "Mở",   desc: "Đổi góc nhìn/phối cảnh; giữ tỷ lệ khối & nội thất, kiến trúc đổi ~40%.",
...
const GEOMETRY_MATRIX = {
  ...
  furniture_style:     [true,  true,  false, true ],
  proportion_detailing:[true,  true,  true,  true ],
...
const STYLE_INTENSITY_CLAUSES = [
      ...
      "Apply the target style at maximum intensity: a bold, confident and expressive interpretation that pushes the materials, colour, textures, lighting and mood to the fullest. This maximum intensity governs ONLY surface styling (materials, colour, textures, lighting, mood) — it must NEVER reshape, distort, simplify or reinterpret the geometry, structure, proportions, ceiling height or camera. Keep every shape and form exactly as defined by the geometry settings; only the styling strength is dialled up.",
    ];
...
- ${["Strictly preserve the MODEL's geometry, camera, walls, openings and every element's position; only swap surface materials, colors and lighting.",
      ...
      "Treat the MODEL as THIS room shown from a FRESH viewpoint: you may freely choose a completely NEW camera angle and perspective. However the building mass, footprint, overall room proportions and ceiling height stay FIXED, and the furniture pieces and their layout/arrangement stay the SAME as the MODEL — only seen from the new angle. Architectural surface detailing may be reinterpreted MODERATELY (roughly 40% change at most, keeping about 60% clearly recognisable) — do NOT push it to the maximum. The result must read as the SAME room and the same furniture, simply re-photographed from a new viewpoint — not a different space."][geometry]}`
      : `\n\n${[
...
const APP_VERSION = "v28.18 · 11/06/2026";
...
export default function InteriorPromptAgent() {
  ...
  function effectivePlatformGuide() {
    ...
    if (geometry === 0) {
      ...
      guide = `a creative TEXT-TO-IMAGE generation prompt for gpt-image (NO base image, NOT using --params). This is the SAME room described in the analysis, re-photographed from a NEW camera angle and a fresh perspective (do NOT reuse the original viewpoint). HARD CONSTRAINTS: keep the building mass, footprint, overall room proportions and ceiling height exactly as described in the analysis, and keep the SAME furniture pieces in the SAME layout/arrangement as the analysis — only the viewpoint changes. Architectural surface detailing may be reinterpreted MODERATELY (about 40% change at most, keeping roughly 60% clearly recognisable); do NOT redesign the room or push detailing to the maximum. ${styleClause} ${negNote} Produce a cohesive, photorealistic interior of THIS same room and the same furniture, seen from the new viewpoint.`;
    } else {
      const allowed = {
...
Return ONLY a valid JSON object (no markdown/backticks): {"prompt": "the English render prompt"}. Nothing outside the JSON.`;
  ...
  async function renderImage() {
    ...
    // Mức 3 (Lấy cảm hứng) = render SINH MỚI để đổi được góc máy: gọi endpoint
    // images/generations (KHÔNG ảnh đầu vào) -> identity phòng + style đi qua
    // prompt. Mức 0-2 = images/edits: gửi MODEL (nền/geometry) + STYLE làm pixel base.
    const generate = geometry === 3;
    const images = [];
    if (!generate) {
    ...
    try {
      const response = await fetch(IMAGE_API_URL, {
        ...
        body: JSON.stringify({ model: "gpt-image-2", prompt, images, size: AR_TO_SIZE[aspectRatio] || "auto", mode: generate ? "generate" : "edit" }),
      });
                  ...
                  <div className="rounded-xl p-3 text-xs leading-snug" style={{ background: C.panel, border: `1px solid ${C.lineSoft}` }}>
                    ...
                    <div className="space-y-2">
                      {history.map((h, idx) => {
                        ...
                        const histPlatform = h.params ? platformName(h.params.platform) : null;
