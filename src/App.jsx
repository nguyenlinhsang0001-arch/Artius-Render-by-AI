...
const APP_VERSION = "v31.1 · 13/06/2026";
...
Return ONLY a valid JSON object (no markdown/backticks): {"prompt": "the English render prompt"}. Nothing outside the JSON.`;
  ...
  async function renderImage() {
    ...
    // giờ giữ NGUYÊN camera.) (v31.1) RIÊNG mức Mở (geometry 3) gửi kèm
    // input_fidelity:"low" để nới độ bám pixel -> AI dám tái thiết kế vỏ phòng/
    // kiến trúc mạnh hơn, trong khi camera vẫn được giữ bằng pixel + clause khóa.
    ...
    try {
      const response = await fetch(IMAGE_API_URL, {
        ...
        body: JSON.stringify({ model: "gpt-image-2", prompt, images, size: AR_TO_SIZE[aspectRatio] || "auto", quality: "medium", mode: generate ? "generate" : "edit", ...(geometry === 3 ? { input_fidelity: "low" } : {}) }),
      });
