...
const GEOMETRY_LEVELS = [
  ...
  { value: 3, label: "Lấy cảm hứng",   short: "Mở",   shortNoModel: "Táo bạo",    labelNoModel: "Phá cách",    desc: "Bám góc máy model (xê dịch nhẹ); mở nhất = mức 2 + biến hình vỏ kiến trúc, tỷ lệ, chiều cao trần, khối.",
...
// luôn xử lý input ở high fidelity). GEO_PRESERVE_CLAUSE chỉ áp các mức geometry
// KHÓA (0/1/2). CONCISE_STYLE_CLAUSE áp MỌI mức: ép mô tả style cô đọng/keyword,
...
- ${["Strictly preserve the MODEL's geometry, camera, walls, openings and every element's position; only swap surface materials, colors and lighting.",
      ...
      "Keep the MODEL's camera vantage as the anchor: the SAME viewpoint, with only a slight reframe or focal-length shift allowed — NOT a brand-new angle. This is the MOST OPEN level and INCLUDES everything the moderate level permits: replace furniture and change its arrangement, swap decor and light fixtures, and rework wall/ceiling/floor surface treatments. ON TOP OF THAT, you may BOLDLY transform the architectural envelope itself — wall and ceiling forms, room proportions, ceiling height, massing and structural detailing — more strongly than the moderate level. Only the camera vantage stays anchored to the MODEL; both the furnishings AND the architecture may be substantially reimagined."][geometry]}`
      : `\n\n${[
...
const APP_VERSION = "v29.5 · 12/06/2026";
...
export default function InteriorPromptAgent() {
  ...
  function effectivePlatformGuide() {
    ...
    if (geometry === 0) {
      ...
      guide = `an IMAGE EDITING / RESTYLE instruction for Nano Banana 2 (Gemini 3.1 Flash Image), using the imported image (the 3D model) as the base, NOT using --params. Open with: 'Using the imported MODEL image as the base, keep the same camera vantage and overall framing — you MAY apply only a slight reframe or focal-length shift, not a new angle.' This is the MOST OPEN level: you MAY replace the furniture and rearrange it, swap decor and fixtures and rework surface treatments, AND on top of that BOLDLY transform the architectural envelope itself — wall and ceiling forms, room proportions, ceiling height, massing and structural detailing — more aggressively than a normal restyle (stronger than level 2). Only the camera vantage stays anchored to the MODEL. Styling goal: '${styleClause}' ${negNote} ${CONCISE_STYLE_CLAUSE} End with: 'This is a re-render of the same space from the same vantage, with both its furnishings and its architecture boldly reimagined — photorealistic and finished.'`;
    } else {
      const allowed = {
