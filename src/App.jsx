...
const APP_VERSION = "v38 · 25/06/2026";
      ...
      <style>{`
        ...
        *:focus-visible { outline: 2px solid ${C.accent}; outline-offset: 2px; }
        ...
        @media (min-width: 768px) {
          ...
          .ipa-result-frame  { flex: 1 1 auto; min-height: 0; width: 100%; max-height: calc(100dvh - 200px); display: flex; align-items: center; justify-content: center; overflow: hidden; }
          ...
          /* So sánh 2 ảnh cạnh nhau: lưới + ô ảnh co vừa chiều cao canvas, không cắt (B5 v3) */
          .ipa-side-grid  { flex: 1 1 auto; min-height: 0; max-height: calc(100dvh - 230px); }
          .ipa-side-pane  { min-height: 0; }
          .ipa-side-pane > select { flex: 0 0 auto; }
          .ipa-side-frame { flex: 1 1 auto; min-height: 0; }
          .ipa-side-frame img { max-height: 100%; max-width: 100%; width: auto !important; height: auto; object-fit: contain; }
                    ...
                    {genImg && genStatus !== "generating" && (
                      <div className="ipa-result-body">
                        ...
                        {compareMode === "slider" ? (
                          ...
                          return (
                            <div className="grid grid-cols-2 gap-2 ipa-side-grid" style={{ gridTemplateRows: "minmax(0, 1fr)" }}>
                              {panes.map((pane) => {
                                const pick = pickOf(pane.sel);
                                return (
                                  <div key={pane.key} className="flex flex-col gap-1.5 min-w-0 ipa-side-pane">
                                    ...
                                    <div className="relative rounded-xl overflow-hidden flex items-center justify-center ipa-side-frame" style={{ background: C.bg, border: `1px solid ${C.line}`, minHeight: 120 }}>
                                      {pick ? (
                                        <ZoomImg
                                          src={pick.src}
                                          alt={pick.label}
                                          frameClassName="w-full h-full flex items-center justify-center"
                                          imgClassName="block max-h-full max-w-full object-contain select-none"
                                        />
                                      ) : (
