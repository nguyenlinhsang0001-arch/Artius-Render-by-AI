...
const APP_VERSION = "v34.2 · 19/06/2026";
...
<body>
  ...
  const navRailItems = [
    { Icon: Sparkles, label: "Tạo" },
    { Icon: ImageIcon, label: "Nguồn" },
    { Icon: Layers, label: "Style" },
    { Icon: Palette, label: "Tinh chỉnh" },
  ...
  const navRailEl = (
    <nav
      ...
      aria-label="Điều hướng (đang phát triển)"
    >
      {navRailItems.map(({ Icon, label }, i) => {
        const active = i === 0;
        return (
          <button
            key={label}
            ...
            disabled
            title={`${label} (sắp có)`}
            className="relative w-full flex flex-col items-center justify-center gap-1 py-1.5"
            style={{ color: active ? C.accent : C.railIcon, cursor: "default", opacity: active ? 1 : 0.8 }}
          >
