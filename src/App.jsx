...
Below is the (user-edited) ENGLISH analysis of an interior scene. Treat this analysis as the STYLE SOURCE, not a verbatim script: you SHOULD push its materials, colours, lighting and mood up or down to match the STYLE INTENSITY below instead of reproducing every field literally. Whether the camera and spatial layout stay fixed or may be reinterpreted is decided ONLY by the GEOMETRY LOCK below — do NOT hard-lock the viewpoint on your own. Build ONE optimized ENGLISH render prompt for platform "${platform}". Format: ${effectivePlatformGuide()} Use aspect ratio ${aspectRatio} and avoid-list "${(negativePrompt || NEGATIVE_BY_PLATFORM[platform] || DEFAULT_NEGATIVE).trim()}", embedded as the format requires (do not drop them).${geometryGuidance}${intensityGuidance}${roomNote}
