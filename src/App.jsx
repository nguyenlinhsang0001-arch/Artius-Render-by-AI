...
// (v30) Field PHỤ cho BLEND trên gpt-image-2: kho điểm nhấn của style PHỤ giữ
// RIÊNG (không hòa vào analysis chính). Ratio được áp lúc build prompt qua
// blendTier -> rebuild text-only re-allocate được mà không cần analyze lại.
const BLEND_ACCENT_KEYS = [
  ["blend_secondary_accents", "Blend · kho điểm nhấn style PHỤ"],
];

// (v30) BLEND TIER cho gpt-image-2 — model KHÔNG hiểu %, chỉ nghe theo SỐ LƯỢNG
// descriptor + nhóm category mỗi style chiếm. Map ratio (% style CHÍNH) -> luật
// inject accent ĐẾM ĐƯỢC. Ngưỡng 80/65/50 (sẽ tinh chỉnh sau khi render thử).
function blendTier(ratioA) {
  if (ratioA >= 80) return {
    id: "accent",
    compose: "inject ONLY 1-2 secondary accents (a single signature furniture/decor piece or one small material touch) drawn from the secondary-accent pool. The secondary style must NOT influence the overall colour palette, the primary materials, the lighting or the mood — those stay fully primary-driven.",
  };
  if (ratioA >= 65) return {
    id: "lean",
    compose: "inject 3-4 secondary elements drawn from the secondary-accent pool: about one secondary material, two or three signature pieces, and at most one minor colour accent. The overall palette, lighting and mood stay primary-driven; the secondary reads as a clear but subordinate layer.",
  };
  return {
    id: "even",
    compose: "let BOTH styles co-own the palette, the materials and the signature pieces in a balanced way: blend their colours and materials evenly and draw signature pieces from both, using the secondary-accent pool generously, so neither style dominates — while still resolving into ONE cohesive, harmonious interior (never two competing zones).",
  };
}

// (v30) Câu chỉ dẫn COMPOSE chèn vào instruction cho sonnet. Cả analyze() lẫn
// rebuildPrompt() đều đi qua effectivePlatformGuide() nên dùng CHUNG 1 nguồn:
// PRIMARY analysis là nền, secondary chỉ vào qua kho accent theo luật tier.
function blendComposeClause(ratioA) {
  const t = blendTier(ratioA);
  return ` BLEND COMPOSITION (the target style is a fusion of TWO interior styles): build the render on the PRIMARY style analysis as the foundation, then ${t.compose} The two MUST resolve into a single cohesive interior, never split into separate zones.`;
}
...
const APP_VERSION = "v30.1 · 12/06/2026";
...
export default function InteriorPromptAgent() {
  ...
  async function analyze(recordChanges) {
    ...
    const styleSourceNote = hasStyleImg
      ...
      : blendActive
        ? `No STYLE image was provided. The TARGET STYLE is a deliberate fusion of TWO interior styles, but you MUST keep them SEPARATE in the analysis (the blend ratio is applied later at prompt-build time, not here):\n- PRIMARY style = "${preset.label} — ${preset.brief}"\n- SECONDARY style = "${presetB.label} — ${presetB.brief}"\nDerive EVERY style-related analysis field (style, color_palette, materials, lighting, mood, etc.) from the PRIMARY style ONLY — do NOT fuse the secondary style into those fields. Then capture the SECONDARY style's signature elements SEPARATELY into the "blend_secondary_accents" field: a comma-separated pool of 6-9 distinctive secondary accents (secondary materials & finishes, signature furniture/decor pieces, motifs, colour touches) — NO subject/room/camera words. Keeping them separate lets the blend ratio control how many accents get injected.`
...
Return ONLY a valid JSON object, no markdown/backticks, with this exact shape:
{
  "analysis": {
    ...
    "blend_secondary_accents": "6-9 signature accents of the SECONDARY style ONLY (secondary materials & finishes, signature furniture/decor pieces, motifs, colour touches), comma-separated, NO subject/room/camera words; do NOT merge these into the other fields"` : ""}
  },
  "prompt": "${effectivePlatformGuide()}"
}

Rules:
- "analysis" values and "prompt" MUST be in English. Fill EVERY analysis key shown above.${mjBlend ? " For \"blend_primary_keywords\" / \"blend_secondary_keywords\", keep each style's descriptors SEPARATE (do NOT merge them); these two sets feed the weighted :: parts of the Midjourney prompt." : ""}${blendAccentActive ? " The PRIMARY style drives every style field; keep \"blend_secondary_accents\" SEPARATE (do NOT merge it in) — it feeds the blend at prompt-build time per the BLEND COMPOSITION rule in the prompt instruction." : ""}
...
- Output the complete JSON only, nothing outside it.`;
  ...
  async function rebuildPrompt(recordChanges) {
    ...
    if (blendOn && !mjBlend) allKeys.push(...BLEND_ACCENT_KEYS.map(([k]) => k));
    const enPayload = {};
    ...
    // (v30) Khi blend: nhắc sonnet rằng blend_secondary_accents là KHO accent
    // (không phải field style thường) -> dùng theo BLEND COMPOSITION trong format.
    const blendNote = (blendOn && !mjBlend)
      ? ` One analysis field, "blend_secondary_accents", is NOT a normal style field: it is a separate pool of secondary-style accents. Use the OTHER fields as the PRIMARY base and draw from this pool ONLY as directed by the BLEND COMPOSITION rule in the format instruction below.`
      : "";
...
Return ONLY a valid JSON object (no markdown/backticks): {"prompt": "the English render prompt"}. Nothing outside the JSON.`;
  ...
  function downloadHistoryItem(h, idx) {
    ...
    const labelMap = Object.fromEntries([...STYLE_KEYS, ...MODEL_KEYS, ...MJ_BLEND_KEYS, ...BLEND_ACCENT_KEYS]);
    ...
    const section = (keys, banner, note) =>
      hasKeys(keys)
        ...
        : "";
    const knownKeys = new Set([...STYLE_KEYS, ...MODEL_KEYS, ...MJ_BLEND_KEYS, ...BLEND_ACCENT_KEYS].map(([k]) => k));
    ...
    const grouped =
      ...
      section(BLEND_ACCENT_KEYS, "Kho điểm nhấn style phụ (blend)", "Pool điểm nhấn của style phụ, giữ riêng (không hòa vào phân tích chính). Tỷ lệ trộn quyết định bao nhiêu accent được đưa vào prompt.") +
...
<body>
  ...
  // thêm field kho accent (BLEND_ACCENT_KEYS) -> render editor + so sánh pending.
  ...
  if (snapshot && snapshot.analysis && analysis) {
    const cmpKeys = mjBlendNow ? [...STYLE_KEYS, ...MODEL_KEYS, ...MJ_BLEND_KEYS]
      : blendAccentNow ? [...STYLE_KEYS, ...MODEL_KEYS, ...BLEND_ACCENT_KEYS]
              ...
              <div className="ipa-anim mt-3">
                ...
                {showAnalysis && (
                  <div className="mt-3 ipa-anim">
                    ...
                    {blendAccentNow && analysis.blend_secondary_accents != null && (
                      <>
                        <h2 className="text-base mt-6 mb-3 flex items-center gap-2 font-bold tracking-tight" style={{ color: C.text }}>
                          <Shuffle className="w-4 h-4" style={{ color: C.accent }} /> Kho điểm nhấn style phụ (blend)
                        </h2>
                        <p className="text-xs mb-2 -mt-1.5" style={{ color: C.textDim }}>
                          Pool điểm nhấn của <strong>style phụ</strong>, giữ riêng (không hòa vào phân tích chính). Tỷ lệ trộn quyết định <strong>bao nhiêu</strong> accent được đưa vào prompt — kéo slider rồi bấm "Cập nhật" là đủ, không cần phân tích lại.
                        </p>
                        <div className="rounded-2xl overflow-hidden" style={{ border: `1px solid ${C.line}` }}>
                          {BLEND_ACCENT_KEYS.map(([key, label], i) => (
                            <AnalysisRow key={key} k={key} label={label} i={i} enValue={analysis[key] || ""} onChangeEn={handleChangeEn} />
                          ))}
                        </div>
                      </>
