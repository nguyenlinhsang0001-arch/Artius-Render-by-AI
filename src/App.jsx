...
// =============================================================
const GPT_IMAGE_STRUCTURED_PROMPT_CLAUSE = "Write the final gpt-image-2 prompt as a compact structured render brief with clear uppercase block headings in this exact order: TASK, LOCKED GEOMETRY, ALLOWED CHANGES, TARGET STYLE, MATERIALS BY ZONE, FF&E / DECOR / LANDSCAPE, LIGHTING, PALETTE, RENDER QUALITY, NEGATIVE CONSTRAINTS, OUTPUT. Use concise semicolon-separated directives inside each block. Do not use markdown tables or code fences.";

const GPT_IMAGE_AR_CLAUSE = "For the OUTPUT block, state the requested aspect ratio as {{ARPHRASE}}, while preserving the original framing as closely as possible; do not crop out major architectural or interior elements just to satisfy the ratio.";

const GPT_IMAGE_NEGATIVE_TRANSLATION_CLAUSE = "Translate the avoid-list into explicit NEGATIVE CONSTRAINTS and positive control language: keep vertical lines straight and parallel, keep accurate undistorted perspective, prevent fisheye/lens distortion/perspective drift/warped walls, keep surfaces clean, and avoid the listed artifacts. Avoid-list: {{NEG}}.";
...
const PLATFORM_GUIDE = {
  nanobanana:
    `an IMAGE EDITING / RESTYLE instruction for gpt-image-2, NOT a scene-generation prompt and NOT using --params. ${GPT_IMAGE_STRUCTURED_PROMPT_CLAUSE} Frame it explicitly as editing the MODEL image: open the TASK block with a direct command such as 'Using the imported image as the exact spatial base.' In LOCKED GEOMETRY, preserve camera angle, perspective, framing, vanishing lines, room/building proportions, openings and major object positions. In ALLOWED CHANGES, state that only surface materials, finish language, decorative details, lighting, FF&E/decor/landscape may be restyled according to the geometry level. ${GPT_IMAGE_AR_CLAUSE} ${GPT_IMAGE_NEGATIVE_TRANSLATION_CLAUSE} End the OUTPUT block with: 'photorealistic finished render, PBR materials, global illumination, sharp high resolution, balanced natural exposure.'`,
...
const STYLE_INTENSITY_CLAUSES = [
      ...
      // mood. Dùng mô tả đo được, tránh từ quá chung.
      // ----- 0 — Nhẹ: cường độ thấp nhất, NHƯNG vẫn render hoàn thiện -----
      "Apply the target style at LOW strength. Reproduce the target palette in a restrained, desaturated way with gentle low contrast; use soft even ambient lighting with shallow shadows; render materials cleanly with subtle texture only; keep the mood calm, neutral and uncluttered. The scene must still be a fully finished photorealistic render — only the styling intensity is low, never the level of finish or realism.",
      // ----- 1 — Vừa: tự nhiên, đời thực -----
      "Apply the target style at MODERATE strength. Match the target palette at natural saturation with balanced contrast; use realistic directional lighting with believable soft shadows; render materials with characteristic texture, grain and sheen clearly visible but not exaggerated; give the scene a comfortable, lifelike mood. The style reads plainly while everything stays natural and believable.",
      // ----- 2 — Đậm: style chiếm ưu thế bề mặt, đọc ra rõ -----
      "Apply the target style at HIGH strength. Use rich but natural saturation, noticeably higher contrast and a defined directional key light with distinct highlights and deeper shadows; render materials with tactile texture, grain, sheen and surface detail matched to the target style brief; make the mood pronounced and atmospheric. The style must dominate the surface treatment and read unmistakably without becoming cartoonish or overly vivid.",
      // ----- 3 — Max: editorial/cinematic, nhưng CHỈ surface, KHÔNG đụng geometry -----
      "Apply the target style at MAXIMUM strength: an editorial, expressive interpretation. Drive the palette to its boldest believable expression with strong dramatic contrast; use cinematic high-contrast directional lighting with pronounced highlights, deep shadows and a controlled mood-setting colour cast; render materials at their richest — maximal texture, sheen, depth and detail. This maximum applies ONLY to surface styling (materials, colour, texture, lighting, mood) — it must NEVER reshape, distort, simplify or reinterpret geometry, structure, proportions, ceiling height, camera, massing or object positions; keep every form exactly as the geometry settings define.",
    ];
...
const STYLE_PRESET_AVOID = {
  ...
  quiet_lux: "flashy gold, heavy marble everywhere, crystal chandeliers, obvious opulence, loud patterns, cluttered decor",
  ...
  taiwanese: "too many local motifs at once; use only 2-3 subtle local accents, avoid theme-park nostalgia, avoid heavy dark colonial styling",
  modern: "classical ornament, rustic distressing, boho layering, overly curved organic forms, excessive luxury décor",
  contemporary: "generic plain modern emptiness, outdated 2000s glossy minimalism, excessive ornament, random colour accents",
  ...
  modern_indochine: "heavy dark traditional Indochine overload, excessive oriental motifs, fake gold, clutter, changing modern massing unnecessarily",
  ...
  maximalist: "random mess; keep it curated and intentional, avoid incoherent palettes and accidental clutter",
  memphis: "ordinary contemporary minimalism, muted beige palette, classical ornament, realistic wood-heavy seriousness",
  dark_academia: "horror/gothic fantasy excess, bright Scandinavian palette, modern glossy minimalism, casual boho décor",
};

function stylePresetBrief(preset) {
  if (!preset) return "";
  const avoid = STYLE_PRESET_AVOID[preset.id];
  return avoid ? `${preset.brief}. Avoid: ${avoid}.` : preset.brief;
}
...
const APP_VERSION = "v38-gpt · 24/06/2026 · structured gpt-image-2 prompts";
