...
const APP_VERSION = "v37.0 · 21/06/2026";
...
function UploadBox({ img, onClick, onDrop, inputRef, onChange, onClear, icon, title, subtitle, active }) {
  return (
    <div
      ...
      onDrop={onDrop}
      className={"relative cursor-pointer rounded-2xl flex flex-col items-center justify-center text-center transition-all duration-200 " + (img ? "p-3" : "p-4 min-h-[168px]")}
      style={{
        background: C.panel,
    ...
    >
      ...
      {img ? (
        <img src={img.preview} alt={title} className="w-full max-h-52 rounded-lg object-contain" />
      ) : (
        <>
          {icon}
...
<body>
  ...
  const topBarEl = (
    ...
    >
      <div className="w-full max-w-[1536px] md:max-w-none mx-auto px-4 md:pl-1 h-14 md:h-16 flex items-center gap-3">
      ...
      <style>{`
        ...
        *:focus-visible { outline: 2px solid ${C.accent}; outline-offset: 2px; }
        ...
        @media (min-width: 768px) {
          ...
          .ipa-canvas-scroll { flex: 1 1 auto; min-height: 0; overflow: hidden; padding-right: 0; display: flex; flex-direction: column; align-items: center; }
          .ipa-img-sticky { position: static; }
          /* Ảnh kết quả: auto-scale vừa khít vùng canvas, mọi tỉ lệ, không cuộn (B5 v2) */
          .ipa-result-panel  { flex: 1 1 auto; min-height: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; width: fit-content; max-width: 100%; }
          .ipa-result-body   { flex: 1 1 auto; min-height: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; }
          .ipa-result-frame  { flex: 0 1 auto; min-height: 0; height: 100%; max-width: 100%; display: flex; align-items: center; justify-content: center; overflow: hidden; }
          .ipa-result-frame > img:first-of-type { display: block; width: 100%; height: 100%; object-fit: contain; }
          .ipa-result-caption { flex: 0 0 auto; }
          /* placeholder đang tạo ảnh: fill vùng canvas (không dùng aspect-ratio gây tràn) */
          .ipa-gen-fill { flex: 0 1 auto; min-height: 0; height: 100%; max-width: 100%; }
        }
      `}</style>
            ...
            {/* IMAGE — lên trên cùng khu Kết quả, dính theo cuộn (sticky) */}
                ...
                {platform === "nanobanana" && prompts?.nanobanana && modelImg && (!isDesktop || genImg || genStatus === "generating" || genStatus === "error") && (
                  <div className="mt-3 rounded-2xl p-3 ipa-img-sticky ipa-result-panel" style={{ background: C.bg, border: `1px solid ${C.line}` }}>
                    ...
                    {genStatus === "generating" && (
                      <div className="rounded-xl flex items-center justify-center ipa-gen-fill" style={{ aspectRatio: aspectRatio.replace(":", " / "), background: C.inputBg, border: `1px dashed ${C.lineSoft}`, color: C.textDim }}>
                    ...
                    {genImg && genStatus !== "generating" && (
                      <div className="ipa-result-body">
                        ...
                        <div
                          ...
                          style={{ aspectRatio: aspectRatio.replace(":", " / "), background: C.bg, touchAction: "none", cursor: "pointer", WebkitTouchCallout: "none", WebkitUserSelect: "none", userSelect: "none" }}
